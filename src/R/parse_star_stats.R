...
starLogs <- lapply(starLogFiles, read.delim, header = FALSE, sep = "|",
                   fill = TRUE, strip.white = TRUE, stringsAsFactors = FALSE)
names(starLogs) <- gsub(
...
starLogs.dt[,c("UNIQUE READS:", "MULTI-MAPPING READS:",
               "UNMAPPED READS:") := NULL]

# 2. fix columns that are actually in percent
setnames(starLogs.dt, old = c("Deletion rate per base",
                              "Insertion rate per base"),
         new = c("Deletion rate per base, %", "Insertion rate per base, %"))

# 3. remove percent signs and convert numeric columns
tonumeric <- names(starLogs.dt[, !c("Library", "Started job on",
                                    "Started mapping on", "Finished on"),
                               with = FALSE])
stripPercent <- function(x){gsub("%", "", x, fixed = TRUE)}
starLogs.dt.noPercent <- starLogs.dt[,lapply(.SD, stripPercent),
                                     .SDcols = tonumeric]
starLogs.dt.num <- starLogs.dt.noPercent[,lapply(.SD, as.numeric),
                                         .SDcols = tonumeric]

# 4. convert date columns to POSIX
dateConvert <- function(x) {as.POSIXct(strptime(x, format = '%b %d %H:%M:%S'))}
todate <- names(starLogs.dt[, c("Started job on", "Started mapping on",
                                "Finished on"), with = FALSE])
...
readsPerGeneFiles <- list.files(path = 'output',
...
starLogs.final[ , `Number of reads in genes` := reads.per.gene.total[Library],
                by = Library]
...
# save CSV
write.table(as.data.frame(starLogs.final), paste0(outDir, "/starLogs.tsv"),
